...
auto handle_torch_function_indexing(PyObject* self, PyObject* index, PyObject* val) -> PyObject* {
  ...
  Py_size_t size = PyTuple_GET_SIZE(index_tup.ptr());
  for (Py_size_t i = 0; i < size; i++) {
...
[[noreturn]]
static void extra_args(const FunctionSignature& signature, size_t nargs) {
...
static size_t find_param(FunctionSignature& signature, PyObject* name) {
  size_t i = 0;
...
[[noreturn]]
static void extra_kwargs(FunctionSignature& signature, PyObject* kwargs, size_t num_pos_args) {
  PyObject *key, *value;
  Py_ssize_t pos = 0;
