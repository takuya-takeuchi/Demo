...
int main(int argc, char* argv[]) {
    ...
    // Pipeline logic:
    // filesrc: Read the file
    // decodebin: Automatically detect and decode the video format
    // videoconvert: Convert raw video to a format suitable for encoding
    // x264enc: Encode video to H.264 (using zerolatency tuning for streaming)
    // h264parse: Format the H.264 stream correctly for the sink
    // kvssink: The KVS plugin that handles uploading to AWS
    // fakesink: A dummy sink to complete the pipeline (since kvssink is the actual destination)
    std::string pipelineStr = 
        "filesrc location=\"" + filePath + "\" ! "
        "decodebin ! "
        "videoconvert ! "
        "openh264enc rate-control=1 bitrate=2000 ! "
        "h264parse ! "
        "kvssink stream-name=\"" + streamName + "\"";
