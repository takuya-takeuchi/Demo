...
std::string GetDriveSignature(const std::string& physicalDrivePath, bool& isGpt)
{
    ...
    switch (pDriveLayoutInformationEx->PartitionStyle)
    {
        case PARTITION_STYLE::PARTITION_STYLE_MBR:
        {
            isGpt = false;
            std::stringstream ss;
	        ss << std::hex << std::setw(8)<< std::setfill('0') << (int32_t)driveLayoutInformationEx.get()->Mbr.Signature;
            return ss.str();
        }
...
int32_t main(int32_t argc, const char** argv)
{
    ...
    bool isGpt;
    const auto id = GetDriveSignature(physicalDrivePath, isGpt);
    if (isGpt)
        std::cout << "GPT Drive Identifier: " << id << std::endl;
    else
        std::cout << "MBR Drive Signature: " << id << std::endl;
